...
First, on the {{ CONSOLE_URL }}/topology/ns/{{ GUID }}-project[Topology View^], click **+Add** and, then _From Catalog_:
...
This will create a new `Jaeger` Kubernetes object in your namespace, triggering the Operator to deploy Jaeger. In the {{ CONSOLE_URL }}/topology/ns/{{ GUID }}-project[Topology View^] you'll see Jaeger spin up:
...
You'll need to trigger some HTTP endpoints to generate traces. Access the http://people-{{GUID}}-project.{{ROUTE_SUBDOMAIN}}/datatable.html[graphical person browser powered by the DataTables library we created earlier^].
...
Open the https://jaeger-{{GUID}}-project.{{ROUTE_SUBDOMAIN}}[Jaeger Query UI^]. By default Jaeger uses the same login details as OpenShift, so click the **Login with OpenShift** button, enter your credentials:
...
Reload the https://jaeger-{{GUID}}-project.{{ROUTE_SUBDOMAIN}}[Jaeger Query UI^], and click **Find Traces**. The new trace should appear the top with multiple spans. Click on it to display details:
