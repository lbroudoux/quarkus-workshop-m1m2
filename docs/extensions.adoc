...
[source,sh,role="copypaste"]
----
cd $CHE_PROJECTS_ROOT/quarkus-workshop-labs && git checkout ocp-4.5
----
...
Although your CodeReady workspace is running on the Kubernetes cluster, it's running with a default restricted _Service Account_ that prevents you from creating most resource types. So, in CodeReady, log in with your workshop user. Click on **Login to OpenShift**, and enter your given credentials:
...
You have one project on this server: "{{ GUID }}-project"

Using project "{{ GUID }}-project".
...
Once logged in, click on the name of your project (`{{ GUID }}-project`):
...
[source,sh,role="copypaste"]
----
mvn -q quarkus:list-extensions -f $CHE_PROJECTS_ROOT/quarkus-workshop-labs
----
