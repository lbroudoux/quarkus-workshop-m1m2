...
----
mvn -q quarkus:add-extension -Dextensions="hibernate-orm-panache, jdbc-h2, jdbc-postgresql, resteasy-jsonb" -f $CHE_PROJECTS_ROOT/quarkus-workshop-labs
----
...
We'll first need the definition of eye colors, so let's create an `enum`. Create a new java class file in `src/main/java/org/acme/people/model` called `EyeColor.java`, and add the following enum definition:
...
Our production environment needs a "real" database so let's deploy a Postgres database to OpenShift. Back in the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View^], click **+Add** on the left, on the _Database_ box on the project overview:
...
* **Namespace** (the first one): Select `quarkus-{{ GUID }}-project`
...
This will deploy the database to our new project. Click on the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View^] to see it:
...
Next, add the following variables in `src/main/resources/application.properties`:
...
%prod.quarkus.s2i.base-jvm-image=registry.access.redhat.com/ubi8/openjdk-11<7>
----
...
<7> Build the application based on OpenJDK 11 image
...
Wait for that command to report *replication controller _inventory-xx_ successfully rolled out* before continuing.
...
Back on the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View^], make sure it's done deploying (dark blue circle):
...
Now access the http://people-quarkus-{{ GUID }}-project.{{ROUTE_SUBDOMAIN}}/datatable.html[graphical person browser powered by the DataTables library^]. It should look like:
