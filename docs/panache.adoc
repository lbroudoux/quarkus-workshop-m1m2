...
Our production environment needs a "real" database so let's deploy a Postgres database to OpenShift. Back in the {{ CONSOLE_URL }}/topology/ns/{{ GUID }}-project[Topology View^], click **+Add** on the left, on the _Database_ box on the project overview:
...
* **Namespace** (the first one): Select `{{ GUID }}-project`
...
This will deploy the database to our new project. Click on the {{ CONSOLE_URL }}/topology/ns/{{ GUID }}-project[Topology View^] to see it:
...
Back on the {{ CONSOLE_URL }}/topology/ns/{{ GUID }}-project[Topology View^], make sure it's done deploying (dark blue circle):
...
Now access the http://people-{{GUID}}-project.{{ROUTE_SUBDOMAIN}}/datatable.html[graphical person browser powered by the DataTables library^]. It should look like:
