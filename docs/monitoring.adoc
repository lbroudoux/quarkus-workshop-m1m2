...
On the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View for your project^], click on `+Add`, and choose "Container Image"
...
On the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View for your project^], you'll see prometheus spinning up.
...
[source,sh,role="copypaste"]
----
mvn -q quarkus:add-extension -Dextensions="metrics" -f $CHE_PROJECTS_ROOT/quarkus-workshop-labs
----

[INFO]
====
If you previously added this extension in earlier exercises, then you'll see:

[source,console]
----
👍 Skipping already present extension io.quarkus:quarkus-smallrye-metrics
----
...
Once the build completes, ensure the app completes its redeployment with this command (or watch the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View for your project^])
...
You'll need to trigger the methods that we've instrumented, so http://people-quarkus-{{ GUID }}-project.{{ROUTE_SUBDOMAIN}}/names.html[reopen the name cloud^], which will start producing names (and generating metrics):

image::names.png[names, 400]

Within about 15-30 seconds, Prometheus should start scraping the metrics. Once again, access the http://prometheus-quarkus-{{ GUID }}-project.{{ROUTE_SUBDOMAIN}}[Prometheus UI^]. Start typing in the query box to look for 'acme':
...
Follow the same process as before: On the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View^], click on `+Add`, and choose "Container Image", and fill in the fields:
...
On the {{ CONSOLE_URL }}/topology/ns/quarkus-{{ GUID }}-project[Topology View for your project^], you'll see Grafana spinning up. Once it completes, click on the arrow to access the Grafana UI:
...
* *URL*: `http://prometheus.quarkus-{{ GUID }}-project:9090`
...
Back on the http://grafana-quarkus-{{ GUID }}-project.{{ROUTE_SUBDOMAIN}}[Grafana Home^], select *New Dashboard* to create a new _Dashboard_ to review the metrics.
