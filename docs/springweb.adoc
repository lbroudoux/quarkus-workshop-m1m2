...
Let’s take a look at what the Spring Web structure looks like. You should see lots of static contents(i.e. _HTML, CSS, Images, Javascripts_) and controller Java classes for rendering the Model-View-Controller web presentation layer:

image::spring-web-structure.png[quarkus, 600]
...
Now let’s inject the *compiled* welcome template in the resource class. Open an empty `WelcomeResource.java` class file, and add the following code under the `// TODO: Inject template` comment:
...
Open an empty `welcome.html` in `src/main/resources/templates`, and add the following code:
...
[NOTE]
====
Quarkus Qute Templating Engine can't be integrated with the https://www.thymeleaf.org/[Thymeleaf^] library that Springp Petclinic uses for now. Instead, we need to create static contents such as _CSS, JavaScript, Image, and Fonts_. You can find more details here:

image::quarkus-static-contents.png[quarkus-petclinic, 700]
====

Open an empty `OwnersResource.java` class file, and add the following code:
...
Open an empty `OupsResource.java` class file, and add the following code:
...
You've learned how to simply refactor _Spring Web_ to _Quarkus_ using *Qute Templating Engine*. Note that Qute technology is still considered experimental and you can find more https://quarkus.io/guides/qute-reference[Qute Reference Guide^] on how to use syntax, build blocks, integration, etc.
